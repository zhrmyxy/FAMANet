...
class Visualizer:
    ...
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label, iou=None):
        ...
        # 4. 新增：创建纯二值掩码图像（无原图叠加）
        # 预测掩码（二值图）
        pred_binary = (pred_mask * 255).astype(np.uint8)
        pred_binary_pil = Image.fromarray(pred_binary).convert('L')  # 确保为灰度图

        # 支持集掩码（取第一张的二值图）
        spt_binary = (spt_masks[0] * 255).astype(np.uint8)
        spt_binary_pil = Image.fromarray(spt_binary).convert('L')

        # 4. 合并所有图像（新顺序）
        # merged_pil = cls.merge_image_pair(
        #     spt_pils +  # 支持集原图（新增）
        #     spt_masked_pils +  # 支持集叠加图（蓝色）
        #     [qry_pil] +  # 查询原图（新增）
        #     [pred_masked_pil,  # 预测叠加图（红色）
        #      qry_masked_pil,  # 真值叠加图（红色）
        #      pred_binary_pil,  # 预测二值图（黑白）
        #      spt_binary_pil]  # 支持二值图（黑白）
        # )
        #
        # merged_pil = cls.merge_image_pair(
        #     spt_pils +  # 支持集原图（新增）
